...
# Step 1: Functions
```{r message=FALSE}
#Function returns dataframe used for radiocarbon calibration. Dataframe contains curves ("shcal13"), deltaRs, and delataErrs, the latter two obtained for marine samples only from a table of Calib marine reservoir readings, using the closest lat/long marine reservoir reading by rhumbline to the lat/long of the 14C determination
...
p3<-ggplot(cfr_char,aes(x=targ_age,y=influx_zt10k_sm10_0250_boot)) +
  ...
  ylab(expression(italic("z"))) +
  xlim(0,10000) +
  coord_cartesian(ylim = c(-1.5,1.5), xlim = c(0,10000),clip="off") +
  theme(plot.margin=unit(c(1,1,1,0), "cm"))+
  scale_x_continuous(limits=c(0,10000),breaks=seq(0,10000,by=2000)) +
  theme( axis.line.y= element_line(color = "white"))
...
png("Fig3_CharMap_v3.png",width=6,height=8,units="in",res=600)
...
legend(x = "center",title=expression(bold("Char Influx (z)")),inset = 0,  bty = "n",
       legend = c("  -3     ","-1.5     ","-0.5","0.5","+1.5     ","  +3    "), pt.cex=c(6,3,1,1,3,6),pch=1,
...
for (i in c(1:(length(breaks)-1))){
  mapdata2<-subset(mapdata,datex<breaks[i])
  up_break<-breaks[i+1]
  if (up_break==0){
    up_break<-200
  }
  ...
  par(mar = c(1, 1, 1, 1)) 
  plot(SAshp,main=paste(breaks[i]/1000," - ",breaks[i+1]/1000,"kBP",sep=""))
  len<-0
  ...
  for (m in c(1:len)){
    ...
    points(mapdataneg[m,3],mapdataneg[m,4],col=alpha("royalblue4",0.4),pch=1,cex=abs(mapdataneg[m,5])*2)
    }
  if (m <= length(mapdatapos[,1])){
    points(mapdatapos[m,3],mapdatapos[m,4],col=alpha("red4",0.4),pch=1,cex=mapdatapos[m,5]*2)
  }
...
#  South African C14 
 ...
 # scale_x_continuous(limits=c(0,10000),breaks=seq(0,10000,by=2000)) +
  ...
  geom_text(x=7000, y=0.2, label="West (GCFR) \ub9\u2074C",colour="red4",size=3) +
  coord_cartesian(xlim = c(0,10000),clip="off") +
  scale_y_continuous(position="right") +
  coord_cartesian(xlim = c(0,10000),clip="off")
...
#Si Fig 2 Map absolute differences in number of positive/negative anomalies
```{r}
...
#SI Figure 3: Moving average trendline
```{r}
png("SI_Fig3_MovingAv.png",width=7,height=6,units="in",res=800)
par(mfrow=c(2,2))
...
#SI Figure 4: Comparison between uncalibrated and calibrated mean ages
```{r}
...
png("SI_Fig4_UnCal.png",width=7,height=6,units="in",res=800)
...
# SI Figure 5: Taphonomic correction
```{r}
all_open_correct<-transformSPD(open_SPD,correction = expression(PrDens/(21149.57 * (CalBP + 1788.03)^-1.26)))
allCorrect<-(all_open_correct$grid$PrDens+closed_SPD$grid$PrDens)
srz_open_correct<-transformSPD(open_srz_SPD,correction = expression(PrDens/(21149.57 * (CalBP + 1788.03)^-1.26)))
...
png("SI_Fig5_TaphCorrect.png",width=7,height=6,units="in",res=800)
par(mfrow=c(2,2))
plot(closed_SPD$grid$calBP,allCorrect,type="l",ylim=c(0,0.7),ylab="Frequency",xlab="Years cal BP",main="All")
